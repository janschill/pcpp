...
class StmMap<K, V> implements OurMap<K, V> {
    ...
    public V get(K k) {
        final Holder<V> result = new Holder<>();
        final ItemNode<K, V> bucket = atomic(() -> {
            final TxnRef<ItemNode<K, V>>[] bs = buckets.get();
            final int h = getHash(k), hash = h % bs.length;
            return bs[hash].get();
        });

        ItemNode.search(bucket, k, result);
        return result.value;
    }
    ...
    public V put(K k, V v) {
        int h = getHash(k);
        final TxnRef<ItemNode<K,V>> bucketRef = atomic(() -> {
            final TxnRef<ItemNode<K, V>>[] bs = buckets.get();
        ...
        final ItemNode<K,V> bucket = atomic(() -> bucketRef.get());
        final ItemNode<K,V> newBucket = ItemNode.delete(bucket, k, holder);
        atomic(() -> {
            bucketRef.set(new ItemNode<>(k, v, newBucket));
            if(bucket == newBucket) cachedSize.increment(); //nothing was deleted, increment size
        });
        return holder.get();
    }
    ...
    public V putIfAbsent(K k, V v) {
        ...
        if(!ItemNode.search(bucket, k , holder)) {
            atomic(() -> {
                bucketRef.set(new ItemNode<>(k, v, bucket));
    ...
    public V remove(K k) {
        ...
        atomic(() -> {
            ...
            if(bucket != newBucket) cachedSize.decrement();
        });
        return holder.get();
    }
    ...
    public void forEach(Consumer<K, V> consumer) {
        ...
        for(int i = 0; i < bs.length; i++) {
            ...
            final int _i = i;
            ItemNode<K, V> node = atomic(() -> bs[_i].get());
            while(node != null) {
